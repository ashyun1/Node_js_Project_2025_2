...
public class UnityToNode : MonoBehaviour
{
    ...
    private IEnumerator GetData(string url, System.Action<string> callback) //get요청하는 코루틴함수
    {
    ...
    private IEnumerator PostData(string url, string json, System.Action<string> callback) //get요청하는 코루틴함수
    {
        var webRequest = new UnityWebRequest(url, "POST");                            //웹 요청 get
        var bodyRaw = Encoding.UTF8.GetBytes(json);                           //요청이 돌아올때까지 대기
