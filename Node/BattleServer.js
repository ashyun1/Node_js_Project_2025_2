...
class BattleServer {
    ...
    handleDisconnect(playerId)
    {
        ...
        if (player && player.inBattle)
        {
            ...
            if (battle)
            {
                ...
                //상대방 상태 초기화
                const opponent = this.players.get(opponetId);
                if(opponent)
                {
    ...
    {
        const player = this.player.get(playerId);
        if(player && player.socket.redayState === WebSocket.OPEN)
        {
    ...
    broadcast(data, excludePlayerId = null)
    {
        ...
        {
            if(id !== excludePlayerId && player.socket.redayState === WebSocket.OPEN)
            {
